...
public class AIPlatform {
    ...
    public AIPlatform(String name, String description, String domain, LocalDate releaseDate, int maxConcurrentUsers, String clusterName, int numNodes, int numGPUs, double ramCapacity, LocalDate nextMaintenanceDate, boolean isGPUEnabled) {
        setName(name);
        setDescription(description);
        setDomain(domain);
        setReleaseDate(releaseDate);
        setMaxConcurrentUsers(maxConcurrentUsers);
        setCluster(clusterName, numNodes, numGPUs, ramCapacity, nextMaintenanceDate, isGPUEnabled);
    }
    ...
    private void setCluster(String name, int numNodes, int numGPUs, double ramCapacity, LocalDate nextMaintenanceDate, boolean isGPUEnabled) {
        try {
            this.cluster = new ComputeCluster(name, numNodes, numGPUs, ramCapacity, nextMaintenanceDate, isGPUEnabled);
